...
has no =github-action-commits=, so =release.yml= keeps SHA pins and checksummed
installs by hand (=allow-dirty = ["ci"]=). After =dist generate=, re-apply those
pins and restore =scripts/install-cargo-dist.sh= / =scripts/install-rustup-pinned.sh=
in place of curl-pipe installers, then run the pin gate:
