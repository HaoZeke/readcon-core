...
must match it. ``cargo test --test version_consistency`` fails if any of
the files below drifts, unless that binding is named in
``INTENTIONAL_LAG`` in ``tests/version_consistency.rs`` with a comment
that states why.
...
3. ``pyproject.toml`` (``[project] version`` and ``readcon-chemfiles==X.Y.Z`` extra)

4. ``pyproject.chemfiles.toml`` (``version = "X.Y.Z"``)

5. ``pixi.toml`` (``version = "X.Y.Z"``)

6. ``docs/source/conf.py`` (``release = "X.Y.Z"``)

7. ``src/lib.rs`` (``assert_eq!(VERSION, "X.Y.Z")`` in the version test)

8. ``julia/ReadCon/Project.toml`` (``version = "X.Y.Z"``)

9. ``fortran/ReadCon/fpm.toml`` (``version = "X.Y.Z"``)

10. ``CITATION.cff`` (``version: X.Y.Z``)

``packaging/wrapdb/readcon-core.wrap`` pins a published cxx tarball URL
and hash; it updates after a GitHub Release, not on every crate bump.
...
2. Bump the version in all files listed above.
