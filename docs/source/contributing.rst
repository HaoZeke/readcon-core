...
Versions are tracked in these source files and must stay synchronized
(``scripts/check_version_consistency.sh`` / ``cargo test --test version_consistency``):

1. ``Cargo.toml`` (``version = "X.Y.Z"``; canonical)

2. ``meson.build`` (``version: 'X.Y.Z'``)

3. ``pyproject.toml`` (``version = "X.Y.Z"`` and ``readcon-chemfiles==X.Y.Z`` extra pin)

4. ``pyproject.chemfiles.toml`` (``version = "X.Y.Z"``)

5. ``pixi.toml`` (``version = "X.Y.Z"``)

6. ``julia/ReadCon/Project.toml`` (``version = "X.Y.Z"``)

7. ``fortran/ReadCon/fpm.toml`` (``version = "X.Y.Z"``)

8. ``CITATION.cff`` (``version: X.Y.Z``)

9. ``docs/source/conf.py`` (``release = "X.Y.Z"``)

10. ``src/lib.rs`` (``assert_eq!(VERSION, "X.Y.Z")`` in the version test)
